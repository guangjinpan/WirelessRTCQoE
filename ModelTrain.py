...
class AIModel_TF(nn.Module):
    def __init__(self):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=6, nhead=3, batch_first=True,dropout=0.5,dim_feedforward=128)
    ...
    def forward(self, F):
        ...
        # print(F.shape)
        x = self.transformer_encoder(F)
        
        # tgt = torch.zeros(len(F1), 1, 6)
        # tgt[:,1,1:] = F[:,30:]
        # print(tgt)
...
if __name__ == '__main__':
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        for i, (Feature,QoE) in enumerate(train_loader):
            ...
            loss = nn.MSELoss(reduction='mean')(output,QoE)
            loss.backward()
            optimizer.step()
        ...
        for i, (Feature,QoE) in enumerate(test_loader):
            ...
            loss = nn.MSELoss(reduction='sum')(output,QoE)
            test_avg += loss.item() 
    ...
    mae = mean_absolute_error(data_test[:,6], y_pred_DNN_best)
    print("预测结果：", y_pred_DNN_best)
    print("MAE：", mae)            
    import matplotlib.pyplot as plt
    plt.plot(data_test[:,6])
