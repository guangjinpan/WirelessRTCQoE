...
class MyDataset(Dataset):
    def __init__(self, data_mat, qoe_mat,time_len,fea_len):
        ...
        self.label = qoe_mat.astype(np.float32).reshape((-1,1,4))
    ...
    def __getitem__(self, idx): 
        ...
        # input_F = self.feature[idx,:,:-1]
        # input_Q=np.zeros((self.time_len+4,1))
        # input_Q[:time_len,0] = self.feature[idx,:,-1]
        # label = self.label[idx,:,:] 
        input_F = self.feature[idx,:,:-1]
        input_Q=np.zeros((self.time_len+4,self.fea_len))
        input_Q[:time_len,:] = self.feature[idx,:,:-1]
        label = self.label[idx,:,:]         
...
if __name__ == '__main__':
    ...
    train_dataset = MyDataset(data_train,qoe_train,time_len,6)
    train_loader = DataLoader(dataset=train_dataset,
                                               batch_size=BATCH_SIZE,
                                               shuffle=True)  # shuffle 标识要打乱顺序
    
    test_dataset = MyTestDataset(data_test,qoe_test,time_len,6)
    test_loader = DataLoader(dataset=test_dataset,
                                               batch_size=BATCH_SIZE,
                                               shuffle=False)  # shuffle 标识要打乱顺序
  
    y_pred_DNN=np.zeros((len(data_test),4))
    y_pred_DNN_train=np.zeros((len(data_train),4))
